<?php
    ...
    if(!class_exists("db_conn")){
        ...
        $sql = "SELECT s.*,CONCAT(s.lname , ', ' , s.fname , ' ' , s.mname) as `name`,c.course_code as course, yl.name as year_level FROM `students` s INNER JOIN `course_list` c on s.course_id = c.course_id inner join `year_level` yl on s.yl_id = yl.yl_id where s.sy_id = ?  order by `name` asc";
            $stmt = $conn->connect()->prepare($sql);
           // $stmt_school->bindParam('school_year_id' , $sy_id , PDO::PARAM_INT);
            $stmt->execute([$sy_id]);
        ...
        <table class="table table-hover table-striped table-bordered">
            ...
            <tbody>
                ...
                <tr class="<?=$oo?>">
                    ...
                    <td class="py-0 px-1 align-middle"><?php echo $data['course'] . ' - ' . $data['year_level']; ?></td>
                    <td class="py-0 px-1 align-middle">
                    ...
                    <th class="text-center py-0 px-1 align-middle">
                        <div class="btn-group" role="group">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                <li><a class="dropdown-item view_data" data-id="<?=$data['student_id'] ?>" href="javascript:void(0)">View</a>
                                </li>
                                <li><a class="dropdown-item edit_data" data-id="<?=$data['student_id'] ?>" href="javascript:void(0)">Edit</a>
                                </li>
                                <li><a class="dropdown-item delete_data" data-id="<?=$data['student_id'] ?>"
                                        data-name="<?=$data['student_code'] ?> - <?=$data['name'] ?>" href="javascript:void(0)">Delete</a></li>
                            </ul>
                        </div>
                    </th>
                </tr>
